...
def test_structured_followup_weekly_limit_recommendation_for_family_returns_concrete_cap(atlas):
    result = atlas.build_structured_pri_followup(
        user_message="Qual vc indica pra 2 pessoas e uma crianca?",
        question_key="open_text_followup",
        expected_answer_type="open_text",
        case_summary={"main_issue_hypothesis": "cashflow_pressure"},
        stage="diagnosis_clarification",
    ...
    content = result["content"].lower()
    assert "2 adultos e 1 criança" in content or "2 adultos e 1 crianca" in content
    assert "r$700" in content
    assert "r$250" in content
...
def test_structured_followup_weekly_limit_recommendation_works_even_with_non_open_text_key(atlas):
    result = atlas.build_structured_pri_followup(
        user_message="Qual vc indica pr 2 pessoas e uma criança?",
...
@pytest.mark.asyncio
async def test_period_overview_routes_pri_analise_mes_directly(atlas, monkeypatch):
    phone = "+5511911115600"
    user_id = f"user_{uuid.uuid4().hex}"
    month = atlas._now_br().strftime("%Y-%m")
    ...
    try:
        ...
        cur.execute(
            """INSERT INTO transactions
               (id, user_id, type, amount_cents, category, merchant, occurred_at)
               VALUES (?, ?, 'INCOME', 830100, 'Salário', 'LuizaLabs', ?)""",
            (str(uuid.uuid4()), user_id, f"{month}-05T09:00:00"),
        )
        cur.execute(
            """INSERT INTO transactions
               (id, user_id, type, amount_cents, category, merchant, occurred_at)
    ...
    async def _should_not_route(*_args, **_kwargs):
        raise AssertionError("mini-router não deveria rodar para 'Pri faça uma análise do mês'")

    monkeypatch.setattr(atlas, "_mini_route", _should_not_route)
    monkeypatch.setattr(atlas, "atlas_agent", _StubAtlasAgent([]))

    result = await atlas.chat_endpoint(user_phone=phone, message="Pri faça uma análise do mês")
    content = atlas._normalize_pt_text(result["content"])

    assert "resumo de" in content
    assert "entradas" in content
    assert "saidas" in content
...
@pytest.mark.asyncio
async def test_period_overview_routes_pri_analise_o_meu_mes_directly(atlas, monkeypatch):
    phone = "+5511911115602"
    user_id = f"user_{uuid.uuid4().hex}"
    month = atlas._now_br().strftime("%Y-%m")
    ...
    async def _should_not_route(*_args, **_kwargs):
        raise AssertionError("mini-router não deveria rodar para 'Pri quero que você analise o meu mês'")
